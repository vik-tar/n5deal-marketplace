...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    // Both callbacks are annotated rather than left to inference. `tsc` does
    // infer these correctly here, so the annotations are defensive, not
    // required — kept because they name the augmented `JWT`, `Session` and
    // `User` interfaces (`@/types/next-auth`) directly instead of restating
    // their fields, so the five assignments below cannot drift out of step
    // with those declarations without a compile error.
